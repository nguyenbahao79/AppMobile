...
export default function PayosQrModal({ visible, qrCode, amountVnd, paymentQrUrl, onCheckStatus, onPaid, onCancel }: Props) {
  ...
  // Interval định kỳ và sự kiện quay lại foreground có thể cùng gọi runCheck() gần như đồng thời;
  // nếu cả 2 lệnh gọi mạng đang chạy song song cùng lúc và cùng trả PAID, cả 2 sẽ đi qua được
  // guard settledRef (vì nó chỉ được set SAU khi await xong) và cùng gọi onPaid() -> hiện "Xem vé"
  // 2 lần. isCheckingRef chặn 2 lệnh gọi runCheck() chạy chồng lên nhau.
  ...
  useEffect(() => {
    ...
    const runCheck = async () => {
      if (settledRef.current || isCheckingRef.current) return;
